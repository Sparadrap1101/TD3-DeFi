...
class ConnectMetamask extends React.Component {
    ...
    balanceHandler() {
        ...
        window.ethereum.request({method: 'eth_getBalance', params: [accounts.toString(), 'latest']})
        .then(result => {
            this.setState({
    ...
    chainChangedHandler() {
        ...
        .then(result => {
            ...
            if (results === 3) {
                chainName = "Ropsten";
                error = 'Wrong network, please connect on Rinkeby.';
            } else if (results === 1) {
                chainName = "Mainnet";
                error = 'Wrong network, please connect on Rinkeby.';
            } else if (results === 4) {
    ...
    signMsgHandler() {
        if (this.state.addressUser !== "-") {
            const message = "By signing this message you agree our terms and conditions.\nThanks.";
            window.ethereum.request({method: 'personal_sign', params: [message, this.state.addressUser.toString()]});
        } else {
            this.setState({
